...
for my $line (@input) {
	...
	$bash_cmd .= '; do printf "%s" "$i"; sleep $(awk "BEGIN { print 0.05 + rand() * (0.1) }"); done; echo;' . "$line";
	$bash_cmd .= '; sleep 1.5 ';
	$bash_cmd .= "\'";
	...
	my ($stdin, $stdout);
	my $pid = IPC::Open3::open3($stdin, $stdout, $stderr, "asciinema", "rec", "--cols", '85', '--rows', '8', '-c', $bash_cmd, '--append', $cast_file);
